(function(){
  ...
  function initSearch(){
    ...
    open.addEventListener('click',async()=>{dialog.showModal();document.body.classList.add('search-open');input.focus({preventScroll:true});await ensure()});if(close)close.addEventListener('click',()=>dialog.close());dialog.addEventListener('close',()=>{document.body.classList.remove('search-open');open.focus({preventScroll:true})});dialog.addEventListener('click',e=>{if(e.target===dialog)dialog.close()});input.addEventListener('input',render);if(filters)filters.addEventListener('click',e=>{const b=e.target.closest('[data-search-filter]');if(!b)return;active=b.dataset.searchFilter;filters.querySelectorAll('.chip').forEach(x=>x.classList.toggle('active',x===b));render()});
  }
})();
