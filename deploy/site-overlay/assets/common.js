(function(){
  ...
  function initPress(){
    ...
    fetch('/assets/data/prensa.json?v=1').then(r=>r.json()).then(d=>{const notes=d.notas.filter(n=>n.estado==='aprobada'&&n.seccion===route);if(!notes.length)return;const section=document.createElement('section');section.className='section alt press-inline';section.id='analisis-prensa';section.innerHTML=`<div class="wrap"><div class="press-inline-head"><div><span class="eyebrow">Datos para comunicar</span><h2>Análisis y prensa</h2><p>Hallazgos breves elaborados a partir de esta sección.</p></div><a class="more" href="/prensa/">Ver todas las notas →</a></div><div class="press-inline-grid">${notes.map(n=>`<a class="press-card" href="/prensa/${n.slug}/"><span class="press-tag">${n.tema}</span><h3>${n.titulo}</h3><p>${n.bajada}</p><span class="press-meta">${new Date(n.fecha+'T12:00:00').toLocaleDateString('es-AR')}</span></a>`).join('')}</div></div>`;const footer=document.querySelector('footer');footer?footer.before(section):document.body.appendChild(section)}).catch(()=>{});
  }
  function initSearch(){
